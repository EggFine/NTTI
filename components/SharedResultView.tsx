...
export function SharedResultView({ d, s }: SharedResultViewProps) {
  // validate
  if (!d || !s) {
    return <ErrorView message="链接不完整，缺少必要参数。" />;
  }

  const decoded = decodeShareUrl(d, s);
  if (!decoded) {
    return <ErrorView message="链接无效或已被篡改，无法显示结果。" />;
  }
  ...
  if (!result) {
    return <ErrorView message="无法识别该人格类型。" />;
  }
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-5 py-10 sm:py-12 md:py-16">
        ...
        <motion.div className="mt-8" {...fadeUp(0.3)}>
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="text-sm font-medium text-muted tracking-wide px-2">TA的15维度画像</h2>
            <div className="h-px flex-1 bg-divider" />
          </div>
